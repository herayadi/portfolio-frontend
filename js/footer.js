...
const Footer = (() => {
  ...
  function init(data) {
    ...
    if (socialContainer && data.personal) {
      _renderSocial(data.personal);
    }
   ...
   * Render Social Links
   */
  function _renderSocial(personalData) {
    // Icons SVG
    const icons = {
    ...
    if (personalData.linkedin) {
      html += `<a href="${personalData.linkedin}" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">${icons.linkedin}</a>`;
    }
    
    if (personalData.github) {
      html += `<a href="${personalData.github}" target="_blank" rel="noopener noreferrer" aria-label="GitHub">${icons.github}</a>`;
    }

    if (personalData.email) {
      html += `<a href="mailto:${personalData.email}" target="_blank" rel="noopener noreferrer" aria-label="Email">${icons.email}</a>`;
    }
